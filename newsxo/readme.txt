=== Newsxo ===
Contributors: Themeansar
Author: Themeansar
Requires at least: WordPress 6.7
Tested up to: WordPress 6.9
Requires PHP: 7.4
Stable tag: 1.4.8
Version: 1.4.8
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html
Tags: one-column, two-columns ,right-sidebar, flexible-header, custom-background, custom-header, custom-menu, editor-style, featured-images, footer-widgets,  theme-options, threaded-comments, rtl-language-support, translation-ready, full-width-template, custom-logo, blog, news

== Description ==

Newsxo is a fast, clean, modern-looking Best Responsive News Magazine WordPress theme. The theme is fully widgetized, so users can manage the content by using easy to use widgets. Newsxo is suitable for dynamic news, newspapers, magazine, publishers, blogs, editors, online and gaming magazines, newsportals,personal blogs, newspaper, publishing or review siteand any creative website. Newsxo is SEO friendly, WPML,Gutenberg, translation and RTL ready. Live preview : https://demo.themeansar.com/newsxo and documentation at https://docs.themeansar.com/docs/Newsxo/ 

== Installation ==

1. In your admin panel, go to Appearance -> Themes and click the 'Add New' button.
2. Type in Newsxo in the search form and press the 'Enter' key on your keyboard.
3. Click on the 'Activate' button to use your new theme right away.

== Copyright ==

Newsxo WordPress Theme, Copyright (c) 2024, Themeansar
Newsxo is distributed under the terms of the GNU GPLs

== Credits ==

* CSS bootstrap.css
Code and documentation copyright 2011-2016 Twitter, Inc. Code released under the MIT license. Docs released under Creative Commons.

* JS bootstrap.js
Copyright 2011-2018 The Bootstrap Authors (https://github.com/twbs/bootstrap/graphs/contributors)
https://github.com/twbs/bootstrap/blob/master/LICENSE

* Bootstrap navwalker
* Copyright (C) 2007 Free Software Foundation, Inc. <http://fsf.org/>
https://github.com/twittem/wp-bootstrap-navwalker

* Google Fonts  
* Inter
    License: [Google Fonts Licensing](https://fonts.google.com/knowledge/glossary/licensing)  
    Source: [Inter on Google Fonts](https://fonts.google.com/specimen/Inter)

* Outfit
    License: [Google Fonts Licensing](https://fonts.google.com/knowledge/glossary/licensing)  
    Source: [Outfit on Google Fonts](https://fonts.google.com/specimen/Outfit)

== Font Awesome license ==
https://github.com/FortAwesome/Font-Awesome FontAwesome 4.7.0 Copyright 2012 Dave Gandy 
Font License: SIL OFL 1.1 Code License: MIT License http://fontawesome.io/license/`

== Smartmenu ==
======================================
 * Owner : https://github.com/vadikom
 * Copyright (c) Vasil Dinkov, Vadikom Web Ltd. ( https://github.com/vadikom/smartmenus/blob/master/LICENSE-MIT)
 * Licensed under https://github.com/vadikom/smartmenus
 * https://github.com/vadikom/smartmenus/blob/master/src/addons/keyboard/jquery.smartmenus.keyboard.js


License: CC0 Public Domain
License Url: https://pxhere.com/en/license
License Url: https://stocksnap.io/license

== Post Image ==
* https://pxhere.com/en/photo/1088035
* https://pxhere.com/en/photo/709677
* https://pxhere.com/en/photo/1086680
* https://pxhere.com/en/photo/1671750
* https://stocksnap.io/photo/green-field-POIHJNTKDB
* https://pxhere.com/en/photo/1342638

== Changelog ==
= Version 0.1
* Initial release.

= Version 0.2
* Fixed Theme Review Issue.

= Version 0.3
* Fixed Navigation Issue.

= Version 0.4
* Update Theme URI.

= Version 0.5
* Update Screenshot.

= Version 0.6
* Update Design.

= Version 0.7
* Fixed Header Toogle Sidebar Issue.

= Version 0.8
* Fixed Scroll to Top JS.

= Version 0.9
* Fixed Styling Issues.

= Version 1.0.0
* Fixed Styling Issues.

= Version 1.0.1
* Fixed Pages Image Issue.

= Version 1.0.2
* Fixed RTL Styling Issue.

= Version 1.0.3
* Fixed Single Page Style Issue.

= Version 1.0.4
* Fixed News Ticker on Mobile.

= Version 1.0.5
* Fixed Styling Issue.

= Version 1.0.6
* Fixed Single Page Styling Issue.

= Version 1.0.7
* Added Search Result Count.

= Version 1.0.8
* Fixed Styling Issues.

= Version 1.0.9
* Fixed Styling Issues.

= Version 1.1.0
* Fixed Serach Styling Issues.

= Version 1.1.1
* Fixed Responsive Styling Issues.

= Version 1.1.2
* Added Google Fonts Licence

= Version 1.1.3
* Added Starter Sites URL.

= Version 1.1.4
* Added Notice Links.

= Version 1.1.5
* Fixed Typography Fonts.

= Version 1.1.6
* Fixed Notice.

= Version 1.1.7
* Update Notice.

= Version 1.1.8
* Fixed Sidebar Comments Styling.

= Version 1.1.9
* Fixed Typography Fonts.

= Version 1.2.0
* Fixed Hashtag Issue on Offcanvas.

= Version 1.2.1
* Fixed Overlay Color Issue.

= Version 1.2.2
* Fixed RTL Issues.

= Version 1.2.3
* Fixed Breaking News Styling Issues.

= Version 1.2.4
* Fixed Default Heading Font Size.

= Version 1.2.5
* Fixed Ticker RTL Issues.

= Version 1.2.6
* Fixed Styling Issues.

= Version 1.2.7
* Fixed Styling Issues.

= Version 1.2.8
* Fixed Inputs RTL Styling Issues

= Version 1.2.9
* Fixed Security Issues.

= Version 1.3.0
* Added Theme Support Function.

= Version 1.3.1
* Fixed Woocommerce Styling Issues.

= Version 1.3.2
* Added Woocommerce Category on Products.

= Version 1.3.3
* Fixed Product Meta Issue on Search Page

= Version 1.3.4
* Fixed WooCommerce Product Page Styling Issues.

= Version 1.3.5
* Added Navigation On Product Featured Image.

= Version 1.3.6
* Fixed WooCommerce Styling Issues.

= Version 1.3.7
* Fixed WooCommerce Product Price Color Issues.

= Version 1.3.8
* Fixed Shop Page Pagination Styling.

= Version 1.3.9
* Fixed Styling Issues.

= Version 1.4.0
* Added Inline Style Function.

= Version 1.4.1
* Fixed Optimize Issues.

= Version 1.4.2
* Fixed Translating Issue.

= Version 1.4.3
* Fixed Styling Issues.

= Version 1.4.4
* Fixed Styling Issues.

= Version 1.4.5
* Added Newsxo Detail Page URL in Footer.

= Version 1.4.6
* Fixed Post Sharing Issues.

= Version 1.4.7
* Added Customize Edit Button For Sections.

= Version 1.4.8
* Fixed Footer Overlay Image.