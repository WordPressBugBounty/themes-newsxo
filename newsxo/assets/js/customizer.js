...
( function( $ ) {
	...
	wp.customize( 'newsxo_footer_bg_img', function( value ) {
	...
	wp.customize( 'newsxo_footer_overlay_color', function( value ) {
		value.bind( function( newVal ) {
			if(newVal !== ''){
	...
	wp.customize( 'newsxo_footer_text_color', function( value ) {
		value.bind( function( newVal ) {
			if(newVal !== ''){
	...
	wp.customize( 'newsxo_footer_column_layout', function( value ) {
		var colum = 12 / value();
	...
	jQuery('.newsxo-customizer-target-btn').click(function(){
	...
	myCustomizer.state( 'paneVisible' ).bind( function( paneVisible ) {
		if(paneVisible == false){
			$('.newsxo-customizer-edit-icon').addClass('d-none');
		}else{
			$('.newsxo-customizer-edit-icon').removeClass('d-none');
		}
	});
...
( function( $ ) {
	...
	range_live_media_load( 'newsxo_slider_title_font_size', '.bs-slide .inner .title', [ 'font-size' ], 'px !important' );
	range_live_media_load( 'newsxo_tren_edit_title_font_size', '.multi-post-widget .bs-blog-post.three.bsm .title', [ 'font-size' ], 'px !important' );
	range_live_media_load( 'newsxo_footer_main_logo_width', 'footer .bs-footer-bottom-area .custom-logo, footer .bs-footer-copyright .custom-logo', [ 'width' ], 'px' );
	range_live_media_load( 'newsxo_footer_main_logo_height', 'footer .bs-footer-bottom-area .custom-logo, footer .bs-footer-copyright .custom-logo', [ 'height' ], 'px' );
