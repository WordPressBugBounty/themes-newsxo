 ...
 * @return array
 */
function newsxo_body_classes($classes)
{
    ...
    $global_site_mode_setting = newsxo_get_option('global_site_mode_setting');
    $classes[] = $global_site_mode_setting;

    $single_post_featured_image_view = newsxo_get_option('single_post_featured_image_view');
    if ($single_post_featured_image_view == 'full') {
        $classes[] = 'ta-single-full-header';
    }

    $global_hide_post_date_author_in_list = newsxo_get_option('global_hide_post_date_author_in_list');
    ...
    $global_alignment = newsxo_get_option('newsxo_archive_page_layout');
    $page_layout = $global_alignment;
    $disable_class = '';
    $frontpage_content_status = newsxo_get_option('frontpage_content_status');
    ...
    // Check if single.
    if ($post && is_singular()) {
        $post_options = get_post_meta($post->ID, 'newsxo-meta-content-alignment', true);
    ...
    if ( isset( $_COOKIE["newsxo-site-mode-cookie"] ) ) {
        $classes[] = $_COOKIE["newsxo-site-mode-cookie"];
    } else {
    	$classes[] = get_theme_mod( 'newsxo_skin_mode', 'defaultcolor' );
    }

    return $classes;
}

add_filter('body_class', 'newsxo_body_classes');

if (!function_exists('newsxo_target_element')) {
    function newsxo_target_element($type, $type_id, $info_text = '') {
        if($info_text == ''){
        ...
        if ( is_customize_preview() && stripos($_SERVER['REQUEST_URI'], 'customize_messenger_channel') !== false && stripos($_SERVER['REQUEST_URI'], 'customize_changeset_uuid') !== false ) { ?>
            <div class="newsxo-customizer-edit-icon" type="<?php echo esc_attr($type); ?>" type_id="<?php echo esc_attr($type_id); ?>">
                <button class="newsxo-customizer-target-btn" title="<?php echo esc_attr($info_text); ?>">
 ...
 */
function newsxo_pingback_header()
{
...
add_action('wp_head', 'newsxo_pingback_header');
 ...
 * @since newsxo 1.0.0
 */
if (!function_exists('newsxo_get_posts')):
    function newsxo_get_posts($number_of_posts, $category = '0', $search = '', $post_type = 'post') 
    {   
...
if (!function_exists('newsxo_all_posts_title')):
    function newsxo_all_posts_title()
    {
        $args = array(
            'post_type'   => 'post',
            'post_status' => 'publish',
 ...
 * @since  newsxo 1.0.0
 */
if (!function_exists('newsxo_post_format')):
    function newsxo_post_format($post_id)
    {
...
if (!function_exists('newsxo_get_block')) :
    /**
     *
     * @param null
     *
     ...
     *
     */
    function newsxo_get_block($block = 'grid', $section = 'post')
    {
...
/* Display Breadcrumbs */
if (!function_exists('newsxo_excerpt_length')) :
    ...
    function newsxo_excerpt_length($length) {
...
add_filter('excerpt_length', 'newsxo_excerpt_length', 999);


/* Display Breadcrumbs */
if (!function_exists('newsxo_excerpt_more')) :
     ...
     * @since 1.0.0
     */
    function newsxo_excerpt_more($more)
    {
        return '...';
    }
...
add_filter('excerpt_more', 'newsxo_excerpt_more');
 ...
 * @return mixed|string
 */
function newsxo_get_freatured_image_url($post_id, $size = 'newsxo-featured') {
...
if (!function_exists('newsxo_audio_format')):
    function newsxo_audio_format() { ?>
        <div class="audio">
            <?php global $post;
...
if (!function_exists('newsxo_edit_link')) :

    function newsxo_edit_link($view = 'default')
    {
        global $post;
        edit_post_link(
            sprintf(
                ...
                /* translators: %s: Name of current post. Only visible to screen readers */
                    __('Edit <span class="screen-reader-text">%s</span>', 'newsxo'),
                    array(
                        'span' => array(
                            'class' => array(),
                        ),
...
add_filter( 'woocommerce_show_page_title', 'newsxo_hide_shop_page_title' );

function newsxo_hide_shop_page_title( $title ) {
...
function newsxo_post_image_display_type($post) {
    $post_image_height_type = get_theme_mod('post_image_height_type','newsxo_post_img_hei');
    $url = newsxo_get_freatured_image_url($post->ID, 'newsxo-medium');
    if ( $post_image_height_type == 'newsxo_post_img_hei' ) {
        if($url) { ?>
            <div class="bs-blog-thumb lg">
                <figure class="bs-thumb-bg back-img" style="background-image: url('<?php echo esc_url($url); ?>');"></figure>
                <a href="<?php the_permalink(); ?>" class="link-div"></a>
                <?php if ((get_theme_mod('newsxo_post_category',true) == true) && get_theme_mod('blog_post_layout','list-layout') !== 'list-layout'){
                    newsxo_post_categories();
                } ?>
            </div> 
        <?php } else {
            if ((get_theme_mod('newsxo_post_category',true) == true) && get_theme_mod('blog_post_layout','list-layout') !== 'list-layout') {
                newsxo_post_categories();
            }
        }
    }
    elseif ( $post_image_height_type == 'newsxo_post_img_acc' ) {
...
/* Typography Fonts */
if (!function_exists('newsxo_typo_fonts')) {

    function newsxo_typo_fonts() {
...
if ( ! function_exists( 'newsxo_header_color' ) ) :

    function newsxo_header_color() { ?>
        <style type="text/css">
...
add_action('wpb_total_comments','newsxo_comment_count');

//SCROLL TO TOP //
if ( ! function_exists( 'newsxo_scrolltoup' ) ) :
    function newsxo_scrolltoup() {
        $scrollup_layout = get_theme_mod('scrollup_layout','fa-arrow-up');
        $scrollup_enable = get_theme_mod('newsxo_scrollup_enable',true); 
...
function newsxo_dropcap() {
    $newsxo_drop_caps_enable = get_theme_mod('newsxo_drop_caps_enable','false');
    if($newsxo_drop_caps_enable == 'true'){ ?>
        <style>
    ...
    <?php } 
} add_action('wp_head','newsxo_dropcap'); 

if ( ! function_exists( 'newsxo_search_popup' ) ) :
    function newsxo_search_popup() { ?>
...
if ( ! function_exists( 'newsxo_overlay_section' ) ) :
function newsxo_overlay_section() { ?>
    <div class="bs-overlay-section" bs-remove-overlay="true"></div>
<?php } 
endif;
add_action('wp_footer','newsxo_overlay_section');

if ( ! function_exists( 'newsxo_dimension' ) ) {
	
	function newsxo_dimension( $default_value, $output_value, $property ) {
...
if ( ! function_exists( 'newsxo_border_radius' ) ) {
	
	function newsxo_border_radius( $default_value, $output_value) {
...
if (!function_exists('newsxo_box_shadow')):
    function newsxo_box_shadow($shadow_setting){
          $output_box_shadow = '';
 ...
 * @return [string]
 */
function newsxo_media_range( $css_prop, $obj_value, $default, $media = 'desktop', $ext = '' ) {
...
/**
 * Takes Ranges Control applies to newsxo_media_range function and ouputs Full css with @media query
 * @param  [string] $css_prop CSS Property to add
 * @param  [string] $control control / settings
 * @param  [string] $ext css value extension, eg. px, in, pc
 * @return [string]
 */
function newsxo_customizer_value( $control, $css_selector, $css_prop, array $default, $ext = '' ) {
    if ( $control ) {
        $control        = get_theme_mod( $control );
        $return         = '';

        if( is_string( $control ) && is_array( json_decode( $control, true ) ) ){
            $desktop_val    = newsxo_media_range( $css_prop, $control,  $default[0], 'desktop', $ext );
            $tablet_val     = newsxo_media_range( $css_prop, $control, $default[1], 'tablet', $ext );
            $mobile_val     = newsxo_media_range( $css_prop, $control, $default[2], 'mobile', $ext );
...
function newsxo_date_display_type() {
    ...
    $newsxo_date_time_show_type = get_theme_mod('newsxo_date_time_show_type','newsxo_default');

    echo '<div class="top-date ms-1">';
        if ( $newsxo_date_time_show_type == 'newsxo_default' ) {
        if($header_date_enable == true) { ?>
            <span class="day">
                <?php echo date_i18n('j M  Y, D', strtotime(current_time("Y-m-d")));?>
            </span>
            <?php } if ($header_time_enable == true) { ?>
            <span id="time" class="time"></span>
        <?php } } elseif( $newsxo_date_time_show_type == 'wordpress_date_setting') {
            if($header_date_enable == true) { ?>
                <span class="day">
                    <?php echo date_i18n( get_option( 'date_format' ) ); ?>
                </span>
            <?php } if ($header_time_enable == true) { ?>
                <span class="time"> <?php $format = get_option('') . ' ' . get_option('time_format'); print date_i18n($format, current_time('timestamp')); ?></span>
            <?php }
        } 
    echo '</div>';
}
...
if ( class_exists( 'WooCommerce' ) ) {
    ...
    if ( ! function_exists( 'newsxo_show_product_category_before_title' ) ) {
        function newsxo_show_product_category_before_title() {
            global $product;
    ...
    add_action( 'woocommerce_shop_loop_item_title', 'newsxo_show_product_category_before_title', 5 );
    ...
    function newsxo_custom_woo_slider_icons( $options ) {
        // 1. Force Arrows ON
    ...
    add_filter( 'woocommerce_single_product_carousel_options', 'newsxo_custom_woo_slider_icons' );
    
    /* Change WooCommerce Gallery Thumbnails to 5 Columns */
    function newsxo_change_product_gallery_columns() {
        return 5; 
    }
    add_filter( 'woocommerce_product_thumbnails_columns', 'newsxo_change_product_gallery_columns' );

    function newsxo_custom_pagination_icons( $args ) {
    ...
    add_filter( 'woocommerce_pagination_args', 'newsxo_custom_pagination_icons' );
}