(function ($) {
  $(document).ready(function () {
    $('.newsxo-btn-get-started').on('click', function (e) {
      e.preventDefault();
      if (!newsxo_ajax_object.can_install) {
        alert('Sorry, you are not allowed to access this page.');
        return;
      }
      var $btn = $(this);
      $btn.html('Processing.. Please wait').addClass('updating-message');
      $.post(newsxo_ajax_object.ajax_url, {
        action: 'install_act_plugin',
        security: newsxo_ajax_object.install_nonce
      }, function (response) {
        if (response.success) {
  ...
  $(document).on('click', '.notice-get-started-class .notice-dismiss', function () {
    ...
    $.post(ajaxurl, {
      action: 'newsxo_dismissed_notice_handler',
      type: type,
    });
  });
})(jQuery);