...
class featured_post_Widget extends WP_Widget {
	...
	function __construct() {
		$widget_options = array(
			...
			'description' => __( 'Featured Posts', 'newsxo' ),
		);
		parent::__construct( 'featured_post_Widget', __( 'AR: Featured Posts', 'newsxo' ), $widget_options );
	...
	public function form( $instance ) {
		...
		$title = ! empty( $instance['title'] ) ? $instance['title'] : esc_html__( 'Featured Posts', 'newsxo' );
		$cat = ! empty( $instance['cat'] ) ? $instance['cat'] : esc_html__( '', 'newsxo' );
		
		// Widget admin form
		?>
		<p>
			<label for="<?php echo $this->get_field_id( 'title'); ?>"><?php esc_html_e('Title: ', 'newsxo'); ?></label>
			<input class="widefat" type="text" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" value="<?php echo esc_attr( $title ); ?>" />
		</p>
		<p>
			<label>
				<?php esc_html_e( 'Select Category', 'newsxo' ); ?>:
				<?php
					wp_dropdown_categories(
						array(
							'show_option_all' => __( 'All categories', 'newsxo' ),
							'hide_empty'      => 0,
